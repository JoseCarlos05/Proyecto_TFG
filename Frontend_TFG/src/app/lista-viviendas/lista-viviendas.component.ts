...
export class ListaViviendasComponent implements OnInit {
  ...
  todasViviendas: Vivienda[] = []; // Lista original para mantener todas las viviendas
  correo!: string;
  ...
  listarViviendas() {
    if (this.comunidad.id) {
      this.viviendaService.listarViviendasComunidad(this.comunidad.id).subscribe({
        next: data => {
          this.todasViviendas = data; // Lista original
          this.listaViviendas = data; // Lista mostrada
        }
      });
    }
  }
